...
def init_dic(lang):
    principal_words = {}
    other_words = {}
    lang_file = 'hunspell/%s_word_equivalents.tbl' %(lang)
    word_id, word_spelling, principal, lang_id = -1, -1, -1, -1
    fp = open(lang_file, 'rb')
    for line in fp:
        pieces = line.rstrip().split('\t')
        if len(pieces) < 2: continue
        if word_id < 0 and 'word_id' in pieces:
          word_id = pieces.index('word_id')
          word_spelling = pieces.index('word_spelling')
          principal = pieces.index('principal')
          lang_id = pieces.index('lang')
          continue
        word_id_ = int(pieces[word_id])
        word_spelling_ = pieces[word_spelling]
        principal_ = int(pieces[principal])
        lang_ = pieces[lang_id]
        if lang_ != lang: continue
        if principal_: principal_words[word_id_] = (word_spelling_)
        else: other_words[word_spelling_] = word_id_
    fp.close()
    dictionary = set()
...
def abridgement_fix(words):
    #print words;quit()
    i=0 # token counter
    days_of_week = {'Mon':u'Monday','Tue':u'Tuesday','Thu':u'Thursday','Fri':u'Friday','Sat':u'Saturday'}
    months = {'Jan':u'January','Feb':u'February','Mar':u'March','Apr':u'April','Jun':u'June','Jul':u'July','Aug':u'August','Sep':u'September','Oct':u'October','Nov':u'November','Dec':u'December'}
        ...
        for sentence_in in sentences:
            ...
            try:
                ...
                if re.search(u"\p{L}{2,}[\.?!…]\p{L}{2,}",sentence_in):
                    ...
                    continue
                words = re_punct.sub('', sentence_in).split()
                #words = PunktSentenceTokenizer().tokenize(sentence_in)
